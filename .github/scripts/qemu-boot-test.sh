#!/usr/bin/env bash
# QEMU Boot Test for Alya OS ISO
# Tests if the ISO boots successfully in Legacy BIOS or UEFI mode
# Usage: qemu-boot-test.sh --iso <path> --mode <bios|uefi> --output-dir <dir> [--timeout <seconds>]
set -euo pipefail
...
# Helper: send QMP command via Python
qmp_send() {
  local cmd="$1"
  python3 -c "
  ...
  if [[ $ELAPSED -ge $MIN_BOOT_TIME ]] && [[ -f "$SERIAL_LOG" ]] && [[ -s "$SERIAL_LOG" ]]; then
    for pattern in "${BOOT_PATTERNS[@]}"; do
      if grep -qi "$pattern" "$SERIAL_LOG" 2>/dev/null; then
        ...
        RESULT="pass"
        echo "Boot detected! Pattern: '$pattern' ($ELAPSED s)"
        break 2
      fi
    done
  fi
